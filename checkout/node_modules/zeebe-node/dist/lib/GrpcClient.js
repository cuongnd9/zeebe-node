...
class GrpcClient extends events_1.EventEmitter {
    constructor({ basicAuth, connectionTolerance, host, oAuth, options = {}, packageName, protoPath, service, useTLS, }) {
        super();
        ...
        this.handleGrpcError = (stream) => async (err) => {
            ...
            this.emit(exports.MiddlewareSignals.Log.Error, `Grpc Error: ${err.message}`);
            const channelState = await this.watchGrpcChannel();
            this.emit(exports.MiddlewareSignals.Log.Debug, `Grpc Channel state: ${connectivityState[channelState]}`);
        ...
        for (const key in listMethods) {
            if (listMethods[key]) {
                ...
                this[`${methodName}Stream`] = async (data) => {
                    ...
                    stream.on('status', s => this.emit(exports.MiddlewareSignals.Log.Debug, JSON.stringify(s)));
                    return stream;
                };
    ...
    watchGrpcChannel() {
        ...
        return new Promise(resolve => {
            ...
            this.emit(exports.MiddlewareSignals.Log.Error, `Grpc Channel State: ${connectivityState[state]}`);
            const deadline = new Date().setSeconds(new Date().getSeconds() + 300);
